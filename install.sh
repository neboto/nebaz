#!/bin/sh
# nebaz installer — fetches the prebuilt binary for this machine from GitHub
...
#   curl -fsSLO https://raw.githubusercontent.com/neboto/nebaz/main/install.sh
#   less install.sh && sh install.sh
#
# Environment:
#   NEBAZ_VERSION      release tag to install (default: latest), e.g. v0.1.0
#   NEBAZ_INSTALL_DIR  where to put the binary (default: ~/.local/bin)
#   NEBAZ_NO_ATTEST=1  skip the provenance check even when `gh` is present
#   GITHUB_TOKEN        lifts the anonymous API rate limit; needed if the repo
#                       is ever private (any token with read access)
#
# Provenance: every release archive carries a build provenance
...
# SHA-256 check still runs; `gh attestation verify <archive> --repo neboto/nebaz`
...
REPO="neboto/nebaz"
BIN="nebaz"
INSTALL_DIR="${NEBAZ_INSTALL_DIR:-$HOME/.local/bin}"
VERSION="${NEBAZ_VERSION:-}"
TOKEN="${GITHUB_TOKEN:-}"
# Releases cut before the workflow attested its archives (none: nebaz has
# attested every archive from its first tag). Kept so the check has a home.
UNATTESTED_VERSIONS=""
...
if [ "${NEBAZ_NO_ATTEST:-}" = "1" ]; then
  attest_status="skipped (NEBAZ_NO_ATTEST=1)"
...
say "Run '$BIN' to start (needs the Azure CLI 2.54 or newer, logged in with 'az login')."
