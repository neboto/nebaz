...
```bash
cargo build                         # debug
cargo run -- -s vm                  # needs `az login`; -s sub / rg / storage / vnet / kv / id / aks / app / sql / foundry
...
| `src/azure/services/{subscriptions,compute,storage,network,network_edge,network_private,keyvault,identity,aks,app_service,sql,foundry}.rs` | one file per service |
