// ported from neboto-tui src/ui/widgets/service_selector.rs @ d483900
...
mod tests {
    ...
    fn filtering_flattens_and_matches_category() {
        ...
        assert!(state.rows.iter().any(
            |r| matches!(r, SelectorRow::Service(s) if *s == ServiceType::Aks)
        ));
    ...
    fn show_preselects_the_current_service() {
        let mut state = ServiceSelectorState::new();
        state.show(Some(ServiceType::KeyVault));
        assert_eq!(state.selected_service(), Some(ServiceType::KeyVault));
    }
