// ported from neboto-tui src/macros.rs @ d483900
...
//! constantly:
//!
//! 1. **Async.** `@vm` fires a list load, `P` awaits a subscription switch. Replaying keys as a burst would fire the next key into an
...
//! 3. **Incremental search.** `@vnet` typed literally is five keystrokes, and
...
//!
//! The same reasoning covers the two context switches: what matters is
//! *which* subscription/location was selected, not the keys used to find it in
...
pub enum MacroStep {
    /// A complete search query applied in one shot (`@vm`, `tag:env=prod`).
    Search(String),
    ...
    /// Location short name (`westeurope`, or `all`) — stored as a string
    /// because `Location` is a macro-generated enum with no serde derive.
    SwitchLocation(String),
    /// Subscription id.
    SwitchSubscription(String),
    ...
    SwitchService(crate::azure::service::ServiceType),
...
impl MacroStep {
    ...
    pub fn label(&self) -> String {
        match self {
            ...
            MacroStep::SwitchLocation(l) => format!("location {}", l),
            MacroStep::SwitchSubscription(s) => format!("sub     {}", s),
...
/// Keys we deliberately never record. `e` tears the whole TUI down to hand
/// the terminal to `$EDITOR` (`main.rs` drops and rebuilds the event
/// channel), and `q` would end the session mid-macro — neither can be
/// meaningfully resumed from.
...
pub struct MacroRecorder {
    ...
    pub last_service: Option<crate::azure::service::ServiceType>,
    ...
    /// also moves on every `@service` search, and that is already reproduced
    /// by its own step.
...
mod tests {
    ...
    fn semantic_steps_round_trip_and_carry_no_key() {
        let steps = vec![
            MacroStep::Search("@vm".into()),
            MacroStep::SelectId {
                id: "/subscriptions/0/resourceGroups/rg/providers/Microsoft.Compute/virtualMachines/web".into(),
                label: "web".into(),
            },
            MacroStep::SwitchSubscription("11111111-1111-1111-1111-111111111111".into()),
            MacroStep::SwitchLocation("westeurope".into()),
            MacroStep::DetailSection("Tags".into()),
        ];
    ...
    fn summary_names_the_service_the_macro_lands_on() {
        let m = Macro {
            name: "kv-audit".into(),
            steps: vec![
                MacroStep::Search("@sub".into()),
                MacroStep::SwitchLocation("all".into()),
                MacroStep::Search("@kv".into()),
            ],
        };
        assert_eq!(m.summary(), "3 steps → @kv");
    }
